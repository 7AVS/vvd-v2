...
#
# Usage:
#   result_df = run_data_pipeline(spark)
...
# Derived from CAMPAIGNS config. Supports primary_metric and secondary_metric.
...
def extract_experiment(spark):
    """M1: Extract experiment population from tactic_evnt_hist.

    Mirrors Vintage/Vvd vintage_engine.py v2.5 (lines 261-306).
    KEY: MNE is DERIVED from TACTIC_ID via substring(TACTIC_ID, 8, 3).
         CLNT_NO is DERIVED from TACTIC_EVNT_ID via regexp_replace(trim(...), "^0+", "").
    """
...
def extract_card_outcomes(spark):
    """M3a: Card acquisition and card activation from DDWTA_VISA_DR_CRD."""
    ...
    return card_acquisition_df, card_activation_df


def extract_usage_outcomes(spark):
    """M3b: Card usage from DDWTA_T_PT_OF_SALE_TXN."""
...
def extract_wallet_outcomes(spark):
    """M3c: Wallet provisioning from EDW (Teradata).

    Mirrors Vintage/Vvd vintage_engine.py load_token_from_edw() (v2.5, lines 392-417).
    Uses EDW cursor if available, falls back to empty DataFrame.
    CLNT_NO: CAST to INTEGER strips leading zeros, then convert to STRING for join compatibility.
    """
    # Mirrors Vintage/Vvd query exactly — DISTINCT added, column aliased to CLNT_NO
    min_year = min(YEARS)
    wallet_query = f"""
    SELECT DISTINCT
    ...
    try:
        # EDW cursor — same pattern as Vintage/Vvd vintage_engine.py
        cursor = EDW.cursor()
        ...
        # to match tactic_df's CLNT_NO (derived as STRING via regexp_replace)
        wallet_provisioning_df = spark.createDataFrame(
            [(str(int(r[0])), r[1]) for r in results],
            "CLNT_NO STRING, SUCCESS_DT DATE"
        )

        print(f"wallet_provisioning: {wallet_provisioning_df.count():,} events")

    except NameError:
        # EDW object not available in this environment
        print("WARNING: EDW cursor not available. wallet_provisioning will be empty.")
        print("Ensure EDW connection is initialized before running pipeline.")
...
def detect_success(tactic_df, success_dfs):
    """M6: Left join experiment population to success outcomes.

    For each metric, only joins against campaigns that use it (driven by
    METRIC_TO_CAMPAIGNS config). Campaigns that don't use a given metric
    skip the shuffle and get 0/NULL for that column.
    Uses column renaming to avoid join ambiguity.
    """
    result_df = tactic_df
    ...
    for metric_name, success_col_name in METRIC_TO_COLUMN.items():
        ...
        # Join only relevant campaign rows
...
def analyze_universe(result_df):
    """Phase 1: Universe overview — volume, test group splits, SRM checks, campaign distribution."""
...
def analyze_performance(result_df):
    """Phase 2: Campaign performance — success rates, lift, significance."""
...
def analyze_overcontacting(result_df):
    """Phase 3: Overcontacting — frequency, diminishing returns, gap analysis, transitions."""
...
def export_results(result_df):
    """Export campaign performance summary as pandas DataFrame.

    Returns a small aggregated pandas DataFrame suitable for CSV export.
    """
    import pandas as pd
...
def run_data_pipeline(spark):
    """Run the full data pipeline: extract, detect success, persist result.

    Persists tactic_df temporarily during extraction, then persists the
    final result_df for reuse across analysis functions. Intermediate
    success source DataFrames are NOT persisted (used once, then GC'd).

    Returns the persisted result_df.
    """
    # M1: Experiment population
    print("=" * 60)
    print("M1: Extracting experiment population...")
    print("=" * 60)
    tactic_df = extract_experiment(spark)
    tactic_df.persist(StorageLevel.MEMORY_AND_DISK)

    row_count = tactic_df.count()
    unique_clients = tactic_df.select("CLNT_NO").distinct().count()
    campaigns = sorted([r.MNE for r in tactic_df.select("MNE").distinct().collect()])
    date_range = tactic_df.agg(F.min("TREATMT_STRT_DT"), F.max("TREATMT_STRT_DT")).collect()[0]
    ...
    # M3: Success outcomes (not persisted — used once in M6 join)
    print("\nM3a: Extracting card outcomes...")
    card_acquisition_df, card_activation_df = extract_card_outcomes(spark)

    print("\nM3b: Extracting usage outcomes...")
    card_usage_df = extract_usage_outcomes(spark)

    print("\nM3c: Extracting wallet outcomes...")
    wallet_provisioning_df = extract_wallet_outcomes(spark)

    # Pre-filter success DFs to experiment participants only.
    # Eliminates non-experiment rows BEFORE the M6 shuffle — critical for
    ...
    print("\nM6: Detecting success...")
    result_df = detect_success(tactic_df, success_dfs)
    result_df.persist(StorageLevel.MEMORY_AND_DISK)
    result_count = result_df.count()  # Force materialization

    # Release tactic_df — no longer needed
    tactic_df.unpersist()

    print(f"\nResult DataFrame: {result_count:,} rows")
    ...
    print("Data pipeline complete. result_df is persisted and ready for analysis.")
    return result_df


def run_analysis(result_df):
    """Run all analysis phases on the persisted result_df."""
    analyze_universe(result_df)
    analyze_performance(result_df)
    analyze_overcontacting(result_df)
    print("\nAll analysis phases complete.")


def cleanup(result_df):
    """Unpersist result_df to release cluster memory."""
    result_df.unpersist()
...
# HOW TO USE
# ============================================================
# Cell 1: Run data pipeline (once per session, ~10-15 min)
#   result_df = run_data_pipeline(spark)
#
# Cell 2: Run analysis (re-run as many times as you want)
#   run_analysis(result_df)
#   # or individually:
#   analyze_universe(result_df)
#   analyze_performance(result_df)
...
#
# Cell 3: Export
#   pdf = export_results(result_df)
#   pdf.to_csv("/tmp/vvd_v2_summary.csv", index=False)
#
# Cell 4: Cleanup
#   cleanup(result_df)
