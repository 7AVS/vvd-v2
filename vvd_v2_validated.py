...
# ====================================================================
# VVD v2 Validated Pipeline
#
# Combined pipeline + inline validation. Runs the full data pipeline
# with validation gates after each major step (M1, M3, M6).
# No separate audit notebook needed.
#
...
# Cells 2V, 3V, 4V: Inline validation after each pipeline step
...
# ============================================================
# CELL 2V: M1 VALIDATION
# Quick sanity checks on experiment population.
# ============================================================

# VALIDATION: Confirm M1 output is non-empty, has all campaigns, and has no structural defects.
# If any check fails, downstream analysis will be wrong -- print WARNING but don't raise.
print("=" * 60)
print("CELL 2V: M1 VALIDATION")
print("=" * 60)

_v_pass = True
_v_count = tactic_df.count()

# Row count > 0
if _v_count == 0:
    print("FAIL: tactic_df has 0 rows. Pipeline cannot continue.")
    _v_pass = False
else:
    print(f"  Row count: {_v_count:,} -- OK")

# All expected MNEs present
_v_mnes = sorted([r.MNE for r in tactic_df.select("MNE").distinct().collect()])
_v_missing = sorted(set(VVD_MNES) - set(_v_mnes))
if _v_missing:
    print(f"  WARNING: Missing MNEs: {_v_missing}")
    _v_pass = False
else:
    print(f"  All {len(VVD_MNES)} MNEs present -- OK")

# Date range covers expected years
_v_min_dt, _v_max_dt = tactic_df.agg(F.min("TREATMT_STRT_DT"), F.max("TREATMT_STRT_DT")).collect()[0]
_v_years_found = sorted(set([r[0] for r in tactic_df.select(F.year("TREATMT_STRT_DT")).distinct().collect()]))
_v_years_missing = sorted(set(YEARS) - set(_v_years_found))
if _v_years_missing:
    print(f"  WARNING: No data for years: {_v_years_missing} (found: {_v_years_found})")
else:
    print(f"  Date range {_v_min_dt} to {_v_max_dt}, years {_v_years_found} -- OK")

# No NULL TREATMT_END_DT (audit A2)
_v_null_end = tactic_df.filter(F.col("TREATMT_END_DT").isNull()).count()
if _v_null_end > 0:
    print(f"  WARNING: {_v_null_end:,} rows have NULL TREATMT_END_DT -- success detection will miss these")
    _v_pass = False
else:
    print(f"  No NULL TREATMT_END_DT -- OK")

# No duplicates on (CLNT_NO, TACTIC_ID, MNE, TREATMT_STRT_DT) (audit A7)
_v_distinct = tactic_df.select("CLNT_NO", "TACTIC_ID", "MNE", "TREATMT_STRT_DT").distinct().count()
_v_dupes = _v_count - _v_distinct
if _v_dupes > 0:
    print(f"  WARNING: {_v_dupes:,} duplicate rows on (CLNT_NO, TACTIC_ID, MNE, TREATMT_STRT_DT)")
    _v_pass = False
else:
    print(f"  No duplicates at deployment grain -- OK")

print(f"\nM1 VALIDATION: {'PASS' if _v_pass else 'FAIL -- review warnings above'}")
...
# ============================================================
# CELL 3V: M3 VALIDATION
# Quick sanity checks on success outcome DataFrames.
# ============================================================

# VALIDATION: Confirm each success DF has rows and its date range overlaps with treatment windows.
# Empty DFs may be legitimate (e.g., EDW unavailable) but should be flagged.
print("=" * 60)
print("CELL 3V: M3 VALIDATION")
print("=" * 60)

_v3_tactic_dates = tactic_df.agg(F.min("TREATMT_STRT_DT"), F.max("TREATMT_END_DT")).collect()[0]
print(f"Treatment window: {_v3_tactic_dates[0]} to {_v3_tactic_dates[1]}")
print()

for _v3_metric, _v3_sdf in success_dfs.items():
    _v3_cnt = _v3_sdf.count()
    if _v3_cnt == 0:
        print(f"  {_v3_metric}: 0 rows -- WARNING (empty, downstream SUCCESS will be all 0 for this metric)")
    else:
        _v3_dates = _v3_sdf.agg(F.min("SUCCESS_DT"), F.max("SUCCESS_DT")).collect()[0]
        print(f"  {_v3_metric}: {_v3_cnt:,} rows, date range {_v3_dates[0]} to {_v3_dates[1]}")
        # Check overlap with treatment windows
        if _v3_dates[1] < _v3_tactic_dates[0] or _v3_dates[0] > _v3_tactic_dates[1]:
            print(f"    WARNING: No date overlap with treatment windows!")
        else:
            print(f"    Date overlap with treatment windows -- OK")

print(f"\nM3 VALIDATION: Complete (review any warnings above)")
...
# ============================================================
# CELL 4V: M6 VALIDATION
# Quick sanity checks on success detection output.
# ============================================================

# VALIDATION: Confirm SUCCESS flag wiring, date window integrity, and no row explosion from joins.
# These are the most critical checks -- a failure here means results are wrong.
print("=" * 60)
print("CELL 4V: M6 VALIDATION")
print("=" * 60)

_v4_pass = True

# Row count unchanged from M1 (no row explosion from joins)
# result_count was set during M6 materialization above
_v4_m1_count = result_df.select("CLNT_NO", "TACTIC_ID", "MNE", "TREATMT_STRT_DT").distinct().count()
if result_count != _v4_m1_count:
    print(f"  WARNING: Row explosion detected! result_df={result_count:,} but distinct deployments={_v4_m1_count:,}")
    _v4_pass = False
else:
    print(f"  Row count {result_count:,} matches distinct deployments -- OK (no row explosion)")

# SUCCESS flag matches expected metric column per MNE (audit A5)
_v4_mne_to_col = {mne: METRIC_TO_COLUMN[cfg["primary_metric"]] for mne, cfg in CAMPAIGNS.items()}
_v4_a5_pass = True
for _v4_mne, _v4_expected_col in _v4_mne_to_col.items():
    _v4_mne_df = result_df.filter(F.col("MNE") == _v4_mne)
    _v4_mismatches = _v4_mne_df.filter(F.col("SUCCESS") != F.col(_v4_expected_col)).count()
    if _v4_mismatches > 0:
        print(f"  WARNING: {_v4_mne} has {_v4_mismatches:,} rows where SUCCESS != {_v4_expected_col}")
        _v4_a5_pass = False
if _v4_a5_pass:
    print(f"  SUCCESS flag matches expected metric column for all MNEs -- OK")
else:
    _v4_pass = False

# SUCCESS=1 rows have success date within [STRT, END] (audit A6)
_v4_a6_pass = True
for _v4_mne, _v4_cfg in CAMPAIGNS.items():
    _v4_dt_col = f"FIRST_{METRIC_TO_COLUMN[_v4_cfg['primary_metric']]}_DT"
    _v4_succ = result_df.filter((F.col("MNE") == _v4_mne) & (F.col("SUCCESS") == 1))
    _v4_succ_n = _v4_succ.count()
    if _v4_succ_n == 0:
        continue
    _v4_bad = _v4_succ.filter(
        (F.col(_v4_dt_col) < F.col("TREATMT_STRT_DT")) |
        (F.col(_v4_dt_col) > F.col("TREATMT_END_DT")) |
        (F.col(_v4_dt_col).isNull())
    ).count()
    if _v4_bad > 0:
        print(f"  WARNING: {_v4_mne} has {_v4_bad:,}/{_v4_succ_n:,} SUCCESS=1 rows with date outside treatment window")
        _v4_a6_pass = False
if _v4_a6_pass:
    print(f"  All SUCCESS=1 rows have dates within treatment windows -- OK")
else:
    _v4_pass = False

# Success rate summary
print(f"\n  Success rates by campaign (Action group):")
_v4_rates = (
    result_df.filter(F.col("TST_GRP_CD") == ACTION_GROUP)
    .groupBy("MNE")
    .agg(F.count("*").alias("n"), F.sum("SUCCESS").alias("s"))
    .collect()
)
for _v4_r in sorted(_v4_rates, key=lambda r: str(r.MNE)):
    _v4_rate = int(_v4_r.s) / int(_v4_r.n) * 100 if int(_v4_r.n) > 0 else 0
    print(f"    {str(_v4_r.MNE)}: {int(_v4_r.s):,}/{int(_v4_r.n):,} = {_v4_rate:.2f}%")

print(f"\nM6 VALIDATION: {'PASS' if _v4_pass else 'FAIL -- review warnings above'}")
